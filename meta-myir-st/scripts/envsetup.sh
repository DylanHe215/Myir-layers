...
_FORMAT_PATTERN='£-£'
# BASH_SOURCE ==> layers/meta-myir-st/scripts/envsetup.sh
# dirname $(readlink -f ${BASH_SOURCE})=/home/hb/code/myir/layers/meta-myir-st/scripts
...
eula_check() {
    ...
    if [ "$(echo $machine_file | wc -w)" -gt 1 ]; then
        ...
        # eula_file=layers/meta-myir-st/conf/eula/myd-yf13x-emmc
...
{
    # 此时，目录进入build目录
    # _MACH_CONF=layers/meta-myir-st/conf/machine/myd-yf13x-emmc.conf
    ...
    if [ -n "${_MACH_CONF}" ]; then
        ...
        # _LAYERS=#@NEEDED_BSPLAYERS: layers/meta-openembedded/meta-oe layers/meta-openembedded/meta-python
        local _LAYERS=$(grep '^#@NEEDED_BSPLAYERS:' $_MACH_CONF)
        # _BSP=layers/meta-openembedded/meta-oe layers/meta-openembedded/meta-python
